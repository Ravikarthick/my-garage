...
export default function GalleryScreen() {
  ...
  function ReelItem({ item: c, index }) {
    const TOP_H   = Platform.OS === 'ios' ? 110 : 88;  // header zone
    const PHOTO_H = SH * 0.46;                          // photo zone
    const INFO_H  = SH - TOP_H - PHOTO_H - TABBAR_H;   // info zone (remaining)
    const brandCol = c.brand === 'hw' ? '#D85A30' : '#185FA5';

    return (
      <View style={{ width: SW, height: SH, backgroundColor: '#0D0D0D' }}>

        {/* ── ZONE 1: HEADER (name + badges) ─────────────────────────── */}
        <View style={{ height: TOP_H, paddingHorizontal: 20, paddingTop: Platform.OS === 'ios' ? 54 : 32, justifyContent: 'flex-end', paddingBottom: 10 }}>
          ...
          <Text style={st.carName} numberOfLines={1}>{c.name}</Text>
          {!!c.manufacturer && <Text style={[st.makerName, { color: c.brand === 'hw' ? '#FF9060' : '#78B4FF' }]} numberOfLines={1}>{c.manufacturer}</Text>}
        </View>

        {/* ── ZONE 2: PHOTO ───────────────────────────────────────────── */}
        <View style={{ height: PHOTO_H, backgroundColor: '#161616', alignItems: 'center', justifyContent: 'center' }}>
          {c.photo
            ? <>
                {/* blurred bg */}
                ...
                {/* main photo */}
                <Image source={{ uri: c.photo }} style={{ width: SW - 24, height: PHOTO_H - 16 }} resizeMode="contain" />
              </>
        ...
        {/* ── ZONE 3: INFO GRID ───────────────────────────────────────── */}
        <View style={{ flex: 1, backgroundColor: '#0D0D0D', padding: 12 }}>
          <View style={st.infoGrid}>
            ...
            {c.colnum && (
              <View style={[st.infoBox, { backgroundColor: 'rgba(24,95,165,0.25)', borderColor: 'rgba(78,140,220,0.3)' }]}>
          ...
          <View style={{ flexDirection: 'row', gap: 8, marginTop: 8 }}>
            <View style={[st.statusBox, {
              ...
              <View style={{ width: 7, height: 7, borderRadius: 4, backgroundColor: c.status === 'owned' ? '#5DB82A' : c.status === 'wish' ? '#5090D0' : '#D04040', marginRight: 6 }} />
              <Text style={{ color: c.status === 'owned' ? '#90E050' : c.status === 'wish' ? '#78B4FF' : '#FF8080', fontSize: 13, fontWeight: '700' }}>
                {c.status === 'owned' ? 'In Collection' : c.status === 'wish' ? 'Wishlist' : 'Duplicate'}
              </Text>
            </View>
            <TouchableOpacity
              style={st.editBtn}
...
console.log('✅ gallery.tsx — completely rewritten, paging fixed!');
console.log('\nRun: npx expo start --clear');
console.log('\nRoot fix: removed ScrollView from inside FlatList items.');
console.log('Each item is now a plain View with exact height=SH.');
console.log('FlatList pagingEnabled now works correctly — one car per swipe.');
