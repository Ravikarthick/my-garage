...
fs.writeFileSync('app/car/[id].tsx', `import React, { useState, useEffect, useCallback } from 'react';
import {
...
export default function CarForm() {
  ...
  // Modal visibility
  const [modal, setModal] = useState(null); // 'mfg' | 'series' | 'color' | 'colnum' | 'mainline'
  ...
  async function scanCard() {
    try {
      ...
      try {
        ...
        const c = await IM.manipulateAsync(r.assets[0].uri, [{ resize: { width: 800 } }], { compress: 0.5, format: IM.SaveFormat.JPEG, base64: true });
        base64 = c.base64;
      } catch(e) {}
      ...
      const txt = (data?.content?.[0]?.text || '').replace(/[^{]*({.*})[^}]*/s, '$1');
  ...
  const Chip = ({ label, active, onPress, ac, tc }) => (
    <TouchableOpacity
      onPress={onPress}
      style={[s.chip, active && { backgroundColor: ac, borderColor: tc }]}
    >
      <Text style={[s.chipT, active && { color: tc }]}>{label}</Text>
    </TouchableOpacity>
  );

  const Field = ({ label, value, placeholder, onPress, children }) => (
    <View style={s.fieldWrap}>
      <Text style={s.lbl}>{label}</Text>
      <TouchableOpacity style={s.fieldBtn} onPress={onPress} activeOpacity={0.7}>
    ...
    <SafeAreaView style={s.safe}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        >
          ...
          <Text style={s.lbl}>
            Manufacturer
            {mfg ? <Text style={{ color: '#3B6D11' }}> (auto-detected)</Text> : <Text style={{ color: '#A0A09C' }}> (tap to select)</Text>}
          </Text>
          <TouchableOpacity style={[s.fieldBtn, mfg && { borderColor: '#3B6D11', backgroundColor: '#EAF3DE' }, { marginBottom: 14 }]} onPress={() => setModal('mfg')}>
            ...
            <Text style={[s.fieldVal, !mfg && s.fieldPlaceholder, { flex: 1 }]} numberOfLines={1}>{mfg || 'Select manufacturer...'}</Text>
            <Ionicons name="chevron-down" size={14} color="#A0A09C" />
          </TouchableOpacity>
          ...
          <View style={{ flexDirection: 'row', gap: 10, marginBottom: 4 }}>
            <View style={{ flex: 2 }}>
              ...
              <TouchableOpacity style={s.fieldBtn} onPress={() => setModal('series')}>
                <Text style={[s.fieldVal, !series && s.fieldPlaceholder, { flex: 1 }]} numberOfLines={1}>{series || 'Select...'}</Text>
                <Ionicons name="chevron-down" size={14} color="#A0A09C" />
              </TouchableOpacity>
            </View>
            <View style={{ flex: 1 }}>
              ...
              <TextInput
                style={[s.fieldBtn, { paddingVertical: 11 }]}
                value={year}
          ...
          <TouchableOpacity style={[s.fieldBtn, { marginBottom: 10 }]} onPress={() => setModal('color')}>
            <Text style={[s.fieldVal, !color && s.fieldPlaceholder, { flex: 1 }]}>{color || 'Select color...'}</Text>
            <Ionicons name="chevron-down" size={14} color="#A0A09C" />
          </TouchableOpacity>
          ...
          <View style={{ flexDirection: 'row', gap: 10, marginBottom: 10 }}>
            <View style={{ flex: 1 }}>
              ...
              <TouchableOpacity style={s.fieldBtn} onPress={() => setModal('colnum')}>
                <Text style={[s.fieldVal, !colnum && s.fieldPlaceholder, { flex: 1 }]}>{colnum || '4/5'}</Text>
                <Ionicons name="chevron-down" size={14} color="#A0A09C" />
              </TouchableOpacity>
            </View>
            <View style={{ flex: 1 }}>
              ...
              <TouchableOpacity style={s.fieldBtn} onPress={() => setModal('mainline')}>
                <Text style={[s.fieldVal, !mainline && s.fieldPlaceholder, { flex: 1 }]}>{mainline || '32/250'}</Text>
                <Ionicons name="chevron-down" size={14} color="#A0A09C" />
              </TouchableOpacity>
            </View>
          </View>
          ...
          <View style={{ flexDirection: 'row', gap: 10, marginBottom: 14 }}>
            <TouchableOpacity style={[s.photoBox, { flex: 2 }]} onPress={takePhoto}>
              {photo
                ...
                : <><Ionicons name="camera" size={24} color="#6B6B67" /><Text style={s.photoT}>Camera</Text></>
              }
            </TouchableOpacity>
          ...
          <TextInput
            style={[s.fieldBtn, { height: 80, textAlignVertical: 'top', paddingTop: 11, marginBottom: 10 }]}
            value={notes}
            onChangeText={setNotes}
            placeholder="Where you got it, price, condition..."
            placeholderTextColor="#A0A09C"
            multiline
          />
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
          ...
          <View style={s.searchBox}>
            ...
            <TextInput
              style={s.searchInput}
              value={mfgQ}
              onChangeText={t => { setMfgQ(t); setMfgList(searchManufacturers(t)); }}
              placeholder="Search..."
              placeholderTextColor="#A0A09C"
              autoFocus
              autoCorrect={false}
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                style={[s.listRow, mfg === item && s.listRowActive]}
                onPress={() => { setMfg(item); setModal(null); setMfgQ(''); setMfgList(MANUFACTURERS.slice(0,40)); }}
              >
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
          ...
          <View style={s.searchBox}>
            ...
            <TextInput
              style={s.searchInput}
              value={seriesQ}
              onChangeText={t => { setSeriesQ(t); setSeriesList(searchSeries(t)); }}
          ...
          {seriesQ.length > 1 && !seriesList.find(x => x.label.toLowerCase() === seriesQ.toLowerCase()) && (
            <TouchableOpacity
            ...
            renderItem={({ item, index }) => {
              ...
              return (
                <View>
                  ...
                  <TouchableOpacity
                    style={[s.listRow, series === item.label && s.listRowActive]}
                    onPress={() => { setSeries(item.label); setModal(null); setSeriesQ(''); setSeriesList(HW_SERIES); }}
                  >
          ...
          <View style={s.sheet}>
            ...
            <ScrollView contentContainerStyle={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, paddingHorizontal: 20, paddingBottom: 30 }}>
              {COLORS.map(c => (
                <TouchableOpacity
                  key={c}
                  style={[s.colorChip, color === c && { backgroundColor: '#D85A30', borderColor: '#D85A30' }]}
                  onPress={() => { setColor(c); setModal(null); }}
                >
          ...
          <View style={s.sheet}>
            ...
            <View style={{ flexDirection: 'row', gap: 8, paddingHorizontal: 20, marginBottom: 14 }}>
              {SERIES_TOTALS.map(t => (
                <TouchableOpacity
                  key={t}
                  style={{ flex: 1, paddingVertical: 10, borderRadius: 10, borderWidth: 1.5, alignItems: 'center', backgroundColor: colnumTotal === t ? '#1A1A18' : '#F5F4F1', borderColor: colnumTotal === t ? '#1A1A18' : '#E0DEDA' }}
            ...
            <ScrollView contentContainerStyle={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, paddingHorizontal: 20, paddingBottom: 30 }}>
              {Array.from({ length: colnumTotal }, (_, i) => i + 1).map(n => (
                <TouchableOpacity
                  key={n}
                  style={[s.numChip, colnum === n + '/' + colnumTotal && { backgroundColor: '#D85A30', borderColor: '#D85A30' }]}
                  onPress={() => { setColnum(n + '/' + colnumTotal); setModal(null); }}
                >
          ...
          <View style={[s.sheet, { maxHeight: '80%' }]}>
            ...
            <ScrollView contentContainerStyle={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, paddingHorizontal: 20, paddingBottom: 30 }}>
              {Array.from({ length: 250 }, (_, i) => i + 1).map(n => (
                <TouchableOpacity
                  key={n}
                  style={[s.numChip, mainline === n + '/250' && { backgroundColor: '#D85A30', borderColor: '#D85A30' }]}
                  onPress={() => { setMainline(n + '/250'); setModal(null); }}
                >
...
console.log('✅ app/car/[id].tsx — complete clean iOS 26 compatible rewrite!');
console.log('\nAll features kept:');
console.log('  ✅ Brand picker (HW/Matchbox)');
console.log('  ✅ Car name + auto-suggest + history');
console.log('  ✅ Scan Card (AI OCR)');
console.log('  ✅ Manufacturer auto-detect + picker');
console.log('  ✅ Series picker with search + custom');
console.log('  ✅ Year');
console.log('  ✅ Color dropdown (35 colors)');
console.log('  ✅ Series # picker (1/5, 1/6, 1/8, 1/10, 1/12)');
console.log('  ✅ Mainline # picker (1-250)');
console.log('  ✅ Tampo');
console.log('  ✅ Treasure Hunt (None/TH/Super TH)');
console.log('  ✅ Photo (camera + library)');
console.log('  ✅ Status (owned/wishlist/duplicate)');
console.log('  ✅ Notes');
